...
- **LOTG rebrand + launch kit + phone-fit pass (2026-08-30 → 09-01).** App,
  landing + desk re-themed black & white, "Certified Hooper presented by Love
  of the Game" (theme `lotg` = default, `oc_theme_v3` one-time migration; the
  LOTG logo traced from his IG → assets/lotg-{lockup,mark,word}.svg; splash =
  LOTG phase → tick phase). "City Kings" → **Certified Hoopers** everywhere,
  zero 👑 (Aiman: stay away from "kings"). Dead CARTO tiles ("API KEY
  REQUIRED") → keyless OSM tiles + CSS invert for dark themes (`paintTiles`).
  **Header fit fix (09-01):** `#meChip` overflowed on ≤390px phones, which
  widened the mobile layout viewport, threw `.tabbar` off-centre and let the
  page scroll sideways. Fix: `.me{min-width:0;flex:0 1 auto}`,
  `@media(max-width:600px)` chip = 36px avatar-only circle with the ✓ as a
  badge (`.me.vf .av::after`, class toggled where OCP renders), `.stories`
  margin −16 (was −18 = 2px bleed), `.tabbar{left:0;right:0;margin-inline:
  auto}`. QA harness scratchpad/demo/qa.mjs (360/375/390/430; only items
  inside h-scrollers / the map may report "outside"). **Launch kit** =
  `launch.html` — generated by `python3 scratchpad/gen_launch.py <SHA>` from
  repo root: reel, 9-slide carousel, 9 posts, install clip, bio, DM reply,
  copy buttons. Assets pinned to a commit SHA (jsDelivr ≤20MB, masters via
  raw.githubusercontent), page served via raw.githack; `posts.html`,
  `carousel.html`, `pack.html` are older sub-pages — re-pin their SHAs too.
  Tiles/slides/install clip re-render from scratchpad tiles.html/carousel.html/
  install.html (tilecap/carcap/instrender+mixinst) after `demo/tour.mjs`
...
- **COURTS BY THE COMMUNITY — community.html, the bio link (edge v32, 2026-09-04,
  ...
  **tell Aiman** box (→ `bug_report` kind vibe page community). Migration
  ...
  AND ideas. **NO NAMES PUBLIC (2026-09-04, Aiman: "can't have anyone's name
  on it"):** stripped "Aiman" from ALL community.html copy (→ we/us/the team),
